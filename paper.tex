...
In this work we employ the canonical four-index integral \texttt{FCIDUMP} interface,\cite{Knowles1989_FCIDUMP} which is used for DCSD, CCSD(T), and EOM-CCSD calculations. In addition, we test an interface to our pilot implementation of density-fitted local correlation methods,\cite{your_local_corr_ref} currently featuring local MP2, local DCSD, and local ADC(2). These methods employ Pipek--Mezey localization\cite{Pipek1989} of the fragment HF orbitals together with the PAO-domain restricted virtual space and are implemented within the local integrated tensor framework\cite{Kats_LITF} of the \textsc{Molpro} program package.
